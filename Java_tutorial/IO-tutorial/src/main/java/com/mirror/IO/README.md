...
在最开头需要指明的一点就是输入和输出都是文件流，跟普通的打开文件就直接打开了的概念不同。需要找到文件位置，创建流的对象，然后按行读取，（一般是按行）。并且到行尾了还有特殊的读取符号（C语言中的，近代的编程语言都基本上源于C语言，都或多或少的跟那玩意儿有联系）
...
Run如果需要对目录进行复杂的拼接、遍历等操作，使用`Path`对象更方便。
...
实话说有没有眼熟，因为这玩意儿看着不就是当初写静态代理和动态代理的时候干的事么
...
另外在这一章下说下时间和日期的处理，很简单，用到的时候再说，因为跟这个输出和输入流一样。属于内容巨多，但是却没啥屁用的东西，可以说很重要，恰恰是因为很重要，所以都是包装好的，用到的时候再查就行
